...
ExpressTranslate.prototype.interpolate = function (string, interpolations, options) {
  ...
  // Rebuild common safe text html tags for copy
  var tagMaps = ['<br>','<br/>','<p>','</p>','<em>','</em>','<strong>','</strong>']
    .reduce(function (mappings, tag) {
      mappings[tag] = new RegExp(quotemeta(escapeHtml(tag)), 'g');
      return mappings;
    }, {});

  Object.keys(tagMaps).forEach(function (tag) {
    string = string.replace(tagMaps[tag], tag);
  });

  // Replace any escaped quotes or double-quotes
  var puncMaps = ['"', "'"].reduce(function (mappings, char) {
    mappings[char] = new RegExp(quotemeta(escapeHtml(char)), 'g');
    return mappings;
  }, {});

  Object.keys(puncMaps).forEach(function (punc) {
    string = string.replace(puncMaps[punc], punc);
  });

  return string;
};

var stringSubIndex = function (obj, accessor) {
  return accessor.split('.').reduce(function (data, currentKey) {
 ...
 * @param {String} defaultStr (Optional) The default string to use if no translation is found
 ...
 * translations present for that language). If default string is provided,
 * the function will return that instead of the key if no translation is found
 */
ExpressTranslate.prototype.translate = function (lang, key, defaultStr, interpolations, options) {
  if (typeof defaultStr !== 'string') {
    options = interpolations;
    interpolations = defaultStr;
  ...
  var translation = stringSubIndex(this.translations[lang], key);
...
ExpressTranslate.prototype.translator = function (lang) {
  var self = this;
  return function expressTranslate (key, defaultStr, interpolations, options) {
    return self.translate(lang, key, defaultStr, interpolations, options);
  };
};
